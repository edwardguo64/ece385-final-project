...
BYTE SPI_wr(BYTE data) {
	...
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &data, 1, rHIRQ, 0);
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	BYTE reg_val = reg + 2;
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg_val, 0, &reg_val, 0);
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &val, 0, &val, 0);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int n;
	for (n = 0; n < nbytes; n++) {
		return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &data[n], 0, &data[n], 0);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 0, &reg, 0);
	return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 0, &reg, 1, &val, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int n;
	for (n = 0; n < nbytes; n++) {
		return_code = alt_avalon_spi_command(SPI_0_BASE, 0, 0, &reg, 1, &data[n], 0);
	}
